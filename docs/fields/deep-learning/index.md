...
<div class="lesson-meta"><span>DLF01—DLF40</span><span>40 个独立主题</span><span>8 篇正式章</span><span>32 篇待重写草稿</span></div>

> **内容状态说明：**路线中的 40 个主题已经独立规划。第一阶段 8 章已按正式学习文章标准完成重写，并配有阶段总结；其余 32 页仍明确标记为待重写草稿。章节数服从知识前置与阶段产物，后续可以增补必要主题。
...
先看 [学习路线与知识图谱](./roadmap)，再进入 [01 · 张量的形状、步幅与广播](/fields/deep-learning/01-tensor-shape-stride)。完成第 8 章后进入[阶段一总结](/fields/deep-learning/stage-1-review)，把八份产物迁移到新的序列任务。若前置不足，只补阻塞当前实验的最小知识，不把学习变成无限准备。
